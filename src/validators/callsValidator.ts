import Joi from 'joi';
import { FirstSpeaker, MessageMedium, EndBehavior, MessageRole } from '../types/call';
...
export const createCallValidator = Joi.object({
    ...
    inactivityMessages: Joi.array().items(
        Joi.object({
            duration: Joi.string().pattern(/^\d+(\.\d+)?s$/).required(),
            message: Joi.string().required(),
            endBehavior: Joi.string().valid(...Object.values(EndBehavior))
