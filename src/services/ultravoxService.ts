...
export const ultravoxService = {
    async createCall(params: CreateCallParams, userId: string) {
        try {
            ...
            if (axios.isAxiosError(error)) {
                throw new AppError(error.response?.data?.message || 'Failed to create call', error.response?.status || 500);
            }
            throw error;
        }
    },
    ...
    async getCall(callId: string) {
        try {
            ...
            if (axios.isAxiosError(error)) {
                throw new AppError(error.response?.data?.message || 'Failed to get call', error.response?.status || 500);
            }
            throw error;
        }
    },
    ...
    async getCallRecording(callId: string) {
        try {
            ...
            if (axios.isAxiosError(error)) {
                throw new AppError(error.response?.data?.message || 'Failed to get recording', error.response?.status || 500);
            }
            throw error;
        }
    },
