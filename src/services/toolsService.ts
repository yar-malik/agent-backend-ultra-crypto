    ...
    async getAllTools() {
        try {
            ...
            if (axios.isAxiosError(error)) {
                ...
                throw new AppError(error.response?.data?.message || 'Failed to fetch tools', error.response?.status || 500);
            }
            throw error;
        }
    },
    ...
    async getTool(toolId: string) {
        try {
            ...
            if (axios.isAxiosError(error)) {
                ...
                throw new AppError(
                    error.response?.status === 404 ? 'Tool not found' : 'Failed to fetch tool',
                    error.response?.status || 500
                );
            }
    ...
    async updateTool(toolId: string, toolData: any) {
        try {
            ...
            const response = await toolsApi.put(`/api/tools/${toolId}`, {
                name: toolData.name,
                definition: {
                    modelToolName: toolData.definition?.modelToolName || localTool.definition.modelToolName,
                    description: toolData.definition?.description || localTool.definition.description,
                    requirements: {
                        httpSecurityOptions: {
                            options: [{}]
                        }
                    ...
                    http: {
                        baseUrlPattern: toolData.definition?.http?.baseUrlPattern || localTool.definition.http.baseUrlPattern,
                        httpMethod: toolData.definition?.http?.httpMethod || localTool.definition.http.httpMethod
                    },
            ...
            if (axios.isAxiosError(error)) {
                ...
                throw new AppError(
                    error.response?.status === 404 ? 'Tool not found' : 'Failed to update tool',
                    error.response?.status || 500
                );
            }
            console.error('Unexpected error:', error);
    ...
    async deleteTool(toolId: string) {
        try {
            ...
            if (axios.isAxiosError(error)) {
                ...
                throw new AppError(
                    error.response?.status === 404 ? 'Tool not found' : 'Failed to delete tool',
                    error.response?.status || 500
                );
            }
            throw error;
