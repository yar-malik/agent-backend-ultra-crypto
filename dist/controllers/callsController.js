...
exports.getCallRecording =
  exports.getCallMessages =
  exports.listCalls =
  exports.getCall =
  exports.createCall =
    void 0;
    ...
    if (error.response) {
      return next(
        new AppError_1.AppError(
          error.response.data.message || "Ultravox API error",
...
const getCall = async (req, res, next) => {
  ...
  try {
    ...
    if (call.userId.toString() !== req.user.id) {
      return next(
        new AppError_1.AppError("Not authorized to access this call", 403)
      );
    }
    res.status(200).json(call);
  } catch (error) {
    if (
      ((_b = error.response) === null || _b === void 0 ? void 0 : _b.status) ===
      404
    ) {
...
const listCalls = async (req, res, next) => {
  ...
  try {
    ...
    const calls = await ultravoxService_1.ultravoxService.listCalls(
      req.user.id
    );
...
const getCallMessages = async (req, res, next) => {
  ...
  try {
    const { callId } = req.params;
    const messages = await ultravoxService_1.ultravoxService.getCallMessages(
      callId
    );
...
const getCallRecording = async (req, res, next) => {
  ...
  try {
    const { callId } = req.params;
    const recording = await ultravoxService_1.ultravoxService.getCallRecording(
      callId
    );
